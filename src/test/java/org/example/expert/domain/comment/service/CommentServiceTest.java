...
class CommentServiceTest {
    ...
    public void 댓글_삭제(){
        ...
        doNothing().when(commentService).showThrow();       // 첩자로 throw처리
    ...
    public void simple_spy(){
        ...
        Comment commentSpy = spy(Comment.class);        // 실제 객체지만 모킹이 가능하다
        
        given(commentSpy.getId()).willReturn(1L);

        assertNull(commentReal.getId());       // 객체는 생성되어있는데, null값으로 들어가있는것!
